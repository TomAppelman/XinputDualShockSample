...
void InitTimer(){
	...
	if (QueryPerformanceFrequency((LARGE_INTEGER*)&frequency)){
		TimerData.hasPC = 0;
		TimerData.resolution = 1.0 / (double)frequency;
	}
	else{
		TimerData.hasPC = 1;
		TimerData.resolution = 0.001; // winmm resolution is 1 ms
	}
	TimerData.base = GetRawTime();
}
...
double appGetTimeMs(){
	return ((double)(GetRawTime() - TimerData.base) * TimerData.resolution) * 1000;
}
